...
for i in range(10000):
	randname1 = random.choice(chrom2)
	randname2 = random.choice(chrom2)
...
realndn = []
for i in range(1, len(chrom2)):
	realndn.append(mandist(chrom2[i-1], chrom2[i]))
...
realneighbors = []
groupsize = 20
for i in range(groupsize, len(chrom2)):
	realneighbors.append(mindiff(chrom2[i-groupsize:i]))
...
realneighbors2 = []
for i in range(groupsize, len(chrom2), groupsize):
	realneighbors2.append(mindiff(chrom2[i-groupsize:i]))
print('real neighbors (skip):', statistics.mean(realneighbors2), statistics.stdev(realneighbors2))

'''
python3 2chrom.py ~/Code/Research/DATA/at_ime_tissues.txt.gz
'''