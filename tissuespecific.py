...
# list of chromosome 1
chrom1 = []
for name in names:
	if name.startswith('AT1G'): chrom1.append(name)

# list of chromosome 2
chrom2 = []
for name in names:
	if name.startswith('AT2G'): chrom2.append(name)

# list of chromosome 3
chrom3 = []
for name in names:
	if name.startswith('AT3G'): chrom3.append(name)
...
# list of chromosome 5
chrom5 = []
for name in names:
	if name.startswith('AT5G'): chrom5.append(name)

# test lists
genetest = ['AT3G02220.1', 'AT4G04880.1', 'AT4G24050.1', 'AT2G16090.1']

# random pair comparison
randndn = []
for i in range(10000):
	randname1 = random.choice(names)
	randname2 = random.choice(names)
...
for i in range(1, len(names)):
	realndn.append(mandist(names[i-1], names[i]))
...
realneighbors = []
groupsize = 20
for i in range(groupsize, len(names)):
	realneighbors.append(mindiff(names[i-groupsize:i]))
...
realneighbors2 = []
for i in range(groupsize, len(names), groupsize):
	realneighbors2.append(mindiff(names[i-groupsize:i]))
print('real neighbors (skip):', statistics.mean(realneighbors2), statistics.stdev(realneighbors2))

'''
python3 tissuespecific.py ~/Code/Research/DATA/at_ime_tissues.txt.gz
'''