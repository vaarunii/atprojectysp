...
for i in range(10000):
	randname1 = random.choice(chrom1)
	randname2 = random.choice(chrom1)
...
realndn = []
for i in range(1, len(chrom1)):
	realndn.append(mandist(chrom1[i-1], chrom1[i]))
...
realneighbors = []
groupsize = 20
for i in range(groupsize, len(chrom1)):
	realneighbors.append(mindiff(chrom1[i-groupsize:i]))
...
realneighbors2 = []
for i in range(groupsize, len(chrom1), groupsize):
	realneighbors2.append(mindiff(chrom1[i-groupsize:i]))
print('real neighbors (skip):', statistics.mean(realneighbors2), statistics.stdev(realneighbors2))

'''
python3 1chrom.py ~/Code/Research/DATA/at_ime_tissues.txt.gz
'''