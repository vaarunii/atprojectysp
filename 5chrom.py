...
for i in range(10000):
	randname1 = random.choice(chrom5)
	randname2 = random.choice(chrom5)
...
realndn = []
for i in range(1, len(chrom5)):
	realndn.append(mandist(chrom5[i-1], chrom5[i]))
...
realneighbors = []
groupsize = 5
for i in range(groupsize, len(chrom5)):
	realneighbors.append(mindiff(chrom5[i-groupsize:i]))
...
realneighbors2 = []
for i in range(groupsize, len(chrom5), groupsize):
	realneighbors2.append(mindiff(chrom5[i-groupsize:i]))
print('real neighbors (skip):', statistics.mean(realneighbors2), statistics.stdev(realneighbors2))

'''
python3 5chrom.py ~/Code/Research/DATA/at_ime_tissues.txt.gz
'''