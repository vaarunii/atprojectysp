...
for i in range(10000):
	randname1 = random.choice(chrom3)
	randname2 = random.choice(chrom3)
...
realndn = []
for i in range(1, len(chrom3)):
	realndn.append(mandist(chrom3[i-1], chrom3[i]))
...
realneighbors = []
groupsize = 20
for i in range(groupsize, len(chrom3)):
	realneighbors.append(mindiff(chrom3[i-groupsize:i]))
...
realneighbors2 = []
for i in range(groupsize, len(chrom3), groupsize):
	realneighbors2.append(mindiff(chrom3[i-groupsize:i]))
print('real neighbors (skip):', statistics.mean(realneighbors2), statistics.stdev(realneighbors2))

'''
python3 3chrom.py ~/Code/Research/DATA/at_ime_tissues.txt.gz
'''